...
🇮🇷 خوهشا زبان مورد نظر را انتخاب کنید 🇮🇷", 
   ...
   'reply_markup'=>json_encode([
      'inline_keyboard'=>[
	 [
		['text'=>"🇮🇶العربية🇮🇶",'callback_data'=>"arabi"],		['text'=>"🇦🇺English🇦🇺",'callback_data'=>"english"],['text'=>"🇭🇺فارسی🇮🇷",'callback_data'=>"persian"]
	  ]	 
		]
		])
  ]);
 ...
 'parse_mode'=>'html',
   'reply_markup'=>json_encode([
    ...
    ['text'=>"Add me to your Gp ♣️️", 'url'=>"https://telegram.me/$bot_?startgroup=start"]
     ...
     [
     ['text'=>"set lang",'callback_data'=>"st"]
     ]
    ]
    ])
    ]);
...
🎋عدد المجموعات المفعلة ~» $geted",
 'parse_mode'=>'html',
   'reply_markup'=>json_encode([
      'inline_keyboard'=>[
	   [
    ['text'=>"تابع", 'url'=>"https://telegram.me/iimaii"]
    ]
    ]
    ])
    ]);
...
if( !$fwd && $type == "supergroup" && $menglish1 == "❌" && $you != "creator" && $you != "administrator" ){
...
if( !$fwd && $type == "supergroup" && $mpersian1== "❌" or $marabi1 == "❌"&& $you != "administrator" ){
...
'message_id'=>$message->message_id
 ...
 if($edit  && $you != "creator" && $you != "administrator" && !$fwd && $type == "supergroup" && $edit1 == "❌"){
bot('deleteMessage',[
'chat_id'=>$chat_id,
'message_id'=>$message->message_id
]);
 ...
 if($number > 1000 or $numper > 1000 && $you != "creator" && $you != "administrator" && !$fwd && $type == "supergroup" && $long_list1 == "❌"){
 bot('deleteMessage',[
'chat_id'=>$chat_id,
'message_id'=>$message->message_id
]);
...
if($doc   && $you != "creator" && $you != "administrator" && !$fwd && $type == "supergroup" && $gif1 == "❌"){
bot('deleteMessage',[
'chat_id'=>$chat_id,
'message_id'=>$message->message_id
]);
...
if($text && $you != "creator" && $you != "administrator" && in_array($from_id, $ex_silent)){
bot('deleteMessage',[
'chat_id'=>$chat_id,
'message_id'=>$message->message_id
]);
...
if($text == "كتم" && $re && $you != "creator" && $you != "administrator" && !in_array($message->reply_to_message->from->id, $ex_silent)){
bot('sendMessage',[
'chat_id'=>$chat_id,
'text'=>"تم ✅ كتم العضو 👤🔕",
'reply_to_message_id'=>$message->message_id,
...
file_put_contents('dat/silent.txt', "\n" . $message->reply_to_message->from->id, FILE_APPEND);
}

if($text == "كتم" && $you != "creator" && $you != "administrator" && in_array($message->reply_to_message->from->id, $ex_silent)){
bot('sendMessage',[
'chat_id'=>$chat_id,
'text'=>"العضو مكتوم 👤🔕",
'reply_to_message_id'=>$message->message_id,
...
if($text == "افتح الكتم" && $you != "creator" && $you != "administrator" && in_array($message->reply_to_message->from->id, $ex_silent)){
bot('sendMessage',[
'chat_id'=>$chat_id,
'text'=>"تم فتح الكتم 📧🔓",
'reply_to_message_id'=>$message->message_id,
...
$o = file_get_contents('data/silent.txt');
$o = str_replace($message->reply_to_message->from->id,'',$o);
$o = preg_replace("/(^[\r\n]*|[\r\n]+)[\s\t]*[\r\n]+/", "\n", $o);
file_put_contents('data/silent.txt', $o);
}
